    ...
    <PageWrapper>
      <div className="pt-40 pb-32 px-6 md:px-12 bg-[#0f0e0c] min-h-screen">
        <div className="container mx-auto">
          ...
          {/* Zones grid */}
          <FadeIn delay={0.2}>
            <div className="mt-24 border border-white/10 grid grid-cols-1 md:grid-cols-5">
              {zones.map((zone, i) => (
                <div key={i} className="p-8 border-b md:border-b-0 md:border-r border-white/10 last:border-0 hover:bg-white/[0.02] transition-colors">
                  <h3 className="font-serif text-white text-lg mb-6 leading-tight">{zone.title}</h3>
                  <ul className="space-y-3">
                    {zone.items.map((item, j) => (
                      <li key={j} className="flex items-start gap-2 text-white/40 text-xs font-light">
                        <span className="mt-1.5 w-1 h-1 rounded-full flex-shrink-0" style={{ background: GOLD + "80" }} />
                        {item}
                      </li>
                    ))}
                  </ul>
          ...
          {/* Philosophy quote */}
          <FadeIn delay={0.1}>
            <div className="mt-24 py-24 text-center border-t border-white/8">
              ...
              <p className="font-serif text-3xl md:text-5xl text-white/80 leading-snug max-w-3xl mx-auto">
